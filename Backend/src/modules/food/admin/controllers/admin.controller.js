...
export async function getRestaurantCategoriesForMenu(req, res, next) {
    try {
        const { id } = req.params;
        if (!id || !mongoose.Types.ObjectId.isValid(id)) {
            return res.status(400).json({ success: false, message: 'Invalid restaurant id' });
        }
        const data = await adminService.getRestaurantCategoriesForMenu(id, req.query || {});
...
export async function getRestaurantCategoryStatusForMenu(req, res, next) {
    try {
        ...
        const category = await adminService.getRestaurantCategoryStatusForMenu(id, categoryId);
        if (!category) {
            return res.status(404).json({ success: false, message: 'Category not found' });
        }
        res.status(200).json({ success: true, message: 'Category status fetched successfully', data: { category } });
    } catch (error) {
        next(error);
    }
}

export async function getRestaurantItemSlotTimingsForMenu(req, res, next) {
    try {
        const { id } = req.params;
        if (!id || !mongoose.Types.ObjectId.isValid(id)) {
            return res.status(400).json({ success: false, message: 'Invalid restaurant id' });
        }
        const data = await adminService.getRestaurantItemSlotTimingsForMenu(id);
        res.status(200).json({ success: true, message: 'Item slot timings fetched successfully', data });
    } catch (error) {
        next(error);
    }
}
...
export async function deleteCategory(req, res, next) {
    try {
        ...
        // Category deletion now cascades into product deletion as well.
        // Invalidate all food search and restaurant detail caches to avoid stale data.
        await Promise.all([
            invalidateCache('restaurants*'),
            invalidateCache('food_search*'),
            invalidateCache(`restaurant_detail*`),
...
export async function createDeliveryPartner(req, res, next) {
    try {
        const { validateDeliveryRegisterDto } = await import(
            '../../delivery/validators/delivery.validator.js'
        );
        ...
        const performer = await resolveActionPerformerSnapshot(req.user);
        const partner = await adminService.createDeliveryPartnerByAdmin(
            { ...validated, submissionType: 'initial' },
            req.files,
            performer
        );
        ...
        res.status(201).json({
            success: true,
            message: 'Delivery partner created and approved successfully. Driver can log in with this phone via OTP.',
            data: partner
        });
    } catch (error) {
        ...
        if (order && order.userId) {
            ...
            await notifyOwnersWithInbox(
                [{ ownerType: 'USER', ownerId: order.userId }],
                {
                    ...
                    body: `Your refund of ₹${amount} for Order #${order.orderId} has been processed successfully.`,
...
export async function updateDepositPaymentSettings(req, res, next) {
    try {
        const body = { ...req.body };
        if (req.file) {
            const { uploadImageBuffer } = await import('../../../../services/upload.service.js');
