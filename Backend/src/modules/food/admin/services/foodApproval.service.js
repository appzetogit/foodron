...
export async function listPendingFoodApprovals(query = {}) {
    ...
    const foodList = await FoodItem.find(filter)
        ...
        .skip(skip)
        .limit(limit)
        .select('restaurantId categoryName name price otherPrice variants image images foodType approvalStatus requestedAt createdAt itemSlotTimingId')
        .lean();
    ...
    const slotIds = Array.from(
        new Set(
            foodList
                .map((f) => String(f?.itemSlotTimingId || '').trim())
                .filter((id) => mongoose.Types.ObjectId.isValid(id))
        )
    );
    const slotDocs = slotIds.length
        ? await (await import('../../restaurant/models/itemSlotTiming.model.js')).ItemSlotTiming.find({
            _id: { $in: slotIds.map((id) => new mongoose.Types.ObjectId(id)) }
        })
            .select('_id name startTime endTime')
            .lean()
        : [];
    const slotMap = new Map(slotDocs.map((slot) => [String(slot._id), slot]));
    ...
    const foodRequests = foodList.map((f) => ({
        ...
        itemSlotTiming: f.itemSlotTimingId ? slotMap.get(String(f.itemSlotTimingId)) || null : null,
