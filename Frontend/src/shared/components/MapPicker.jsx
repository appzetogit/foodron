...
const MapPicker = ({
  ...
  const shouldEnforceZone = enforceZone && zonePath.length >= 3;

  const rejectOutsideZone = useCallback(
    (lat, lng) => {
      if (!shouldEnforceZone) return false;
      if (isPointInZone(lat, lng, zoneCoordinates)) return false;
      toast.error(
        zoneLabel
          ? `Pin must be inside ${zoneLabel}. Choose a location within the highlighted zone.`
          : "Pin must be inside your selected service zone.",
      );
      return true;
    },
    [shouldEnforceZone, zoneCoordinates, zoneLabel],
  );

  const applyMarkerPosition = useCallback(
    (newPos, formattedAddress = "") => {
      if (rejectOutsideZone(newPos.lat, newPos.lng)) return false;
      setMarker(newPos);
  ...
  const handlePlaceChanged = () => {
    if (autocompleteRef.current) {
      ...
      if (place.geometry) {
        ...
        if (
          !applyMarkerPosition(newPos, place.formatted_address || "")
        ) {
          return;
        }
        if (mapRef.current) {
  ...
  const handleConfirm = async () => {
    ...
    if (rejectOutsideZone(marker.lat, marker.lng)) {
      return;
    }
