...
export default function UserOrderDetails() {
  ...
  // jsPDF's built-in fonts have no rupee glyph, so the symbol prints as "?".
  const formatPdfAmount = (value) => `Rs. ${toAmount(value).toFixed(2)}`

  const billBreakdown = {
    itemTotal: toAmount(pricing.subtotal ?? pricing.itemTotal),
    tax: toAmount(pricing.tax),
    deliveryFee: toAmount(pricing.deliveryFee),
    quickDeliveryFee: toAmount(pricing.quickDeliveryFee),
    packagingFee: toAmount(pricing.packagingFee),
  ...
  const handleDownloadSummary = async () => {
    try {
      ...
      const tableData = items.map(item => [
        ...
        formatPdfAmount(Number(item.price || 0) * Number(item.quantity || item.qty || 1))
      ])
      ...
      // Bill summary mirrors the on-screen breakdown so the printed total reconciles.
      let billY = finalY + 12
      if (billY > 240) {
        doc.addPage()
        billY = 25
      ...
      doc.setFontSize(11)
      doc.setFont('helvetica', 'bold')
      doc.text('Bill Summary', 20, billY)
      billY += 7

      doc.setFontSize(10)
      const billRows = [
        ['Item total', formatPdfAmount(billBreakdown.itemTotal)],
        ['GST (govt. taxes)', formatPdfAmount(billBreakdown.tax)],
        ['Delivery fee', billBreakdown.deliveryFee > 0 ? formatPdfAmount(billBreakdown.deliveryFee) : 'Free'],
      ]
      if (billBreakdown.quickDeliveryFee > 0) {
        billRows.push(['Quick delivery', formatPdfAmount(billBreakdown.quickDeliveryFee)])
      }
      if (billBreakdown.packagingFee > 0) {
        billRows.push(['Packaging', formatPdfAmount(billBreakdown.packagingFee)])
      }
      billRows.push(['Platform fee', formatPdfAmount(billBreakdown.platformFee)])
      ...
      billRows.forEach(([label, value]) => {
        ...
        doc.text(value, 195, billY, { align: 'right' })
        billY += 6
      })

      billY += 2
      doc.setLineWidth(0.3)
      doc.line(20, billY, 195, billY)
      billY += 7

      doc.setFontSize(12)
      doc.setFont('helvetica', 'bold')
      doc.text(`Paid (${paymentMethod.toUpperCase()})`, 20, billY)
      doc.text(formatPdfAmount(billBreakdown.total), 195, billY, { align: 'right' })

      const fileName = `Order_Summary_${orderIdDisplay}.pdf`
      const saved = await savePdfDocument(doc, fileName)

      if (saved) {
        toast.success("Invoice downloaded successfully!")
      } else {
        toast.error("Could not save the invoice on this device")
      }
    } catch (error) {
      ...
      <div className="p-4 space-y-4">
          ...
          {items.map((item, idx) => (
            <div key={idx} className="flex justify-between items-start mt-2">
              ...
              <span className="text-sm text-gray-800 font-medium">
                ₹{(toAmount(item.price) * (Number(item.quantity || item.qty) || 1)).toFixed(2)}
              </span>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="p-4 space-y-2 text-sm">
            ...
            {billBreakdown.packagingFee > 0 && (
              <div className="flex justify-between">
                <span className="text-gray-500">Packaging</span>
                <span className="text-gray-800">
                  ₹{billBreakdown.packagingFee.toFixed(2)}
                </span>
              </div>
            )}
            <div className="flex justify-between">
              ...
              <span className="text-gray-800">
                ₹{billBreakdown.platformFee.toFixed(2)}
              </span>
            </div>
            {billBreakdown.subscriptionFee > 0 && (
              <div className="flex justify-between">
                <span className="text-gray-500">Subscription / other fees</span>
                <span className="text-gray-800">
                  ₹{billBreakdown.subscriptionFee.toFixed(2)}
                </span>
              </div>
            )}
