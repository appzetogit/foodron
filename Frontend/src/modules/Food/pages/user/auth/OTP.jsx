...
export default function OTP() {
  ...
  const handleVerify = async (otpValue = null) => {
    ...
    try {
      ...
      // Clear any leftover guest cart if this is a newly registered user
      if (data.isNewUser) {
        localStorage.removeItem("food_cart")
        localStorage.removeItem("quick_cart")
      }

      if (needsName) {
        setSessionTokens({ accessToken, refreshToken, user })
  ...
  const handleSubmitName = async () => {
    ...
    try {
      ...
      if (sessionTokens) {
        setUserAuthData("user", sessionTokens.accessToken, updatedUser, sessionTokens.refreshToken)
        window.dispatchEvent(new Event("userAuthChanged"))
      }
