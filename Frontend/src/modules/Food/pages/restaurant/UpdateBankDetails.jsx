...
export default function UpdateBankDetails() {
  ...
  const loadProfile = async () => {
    try {
      ...
      const pending = doc.pendingProfileChanges?.proposed || {}
      
      const rawAccountNum = pending.accountNumber ?? doc.accountNumber
      const accountNumber = String(rawAccountNum || "").replace(/\s|-/g, "")
      
      const rawUpiQrImage = pending.upiQrImage ?? doc.upiQrImage
      const upiQrImage =
        typeof rawUpiQrImage === "string"
          ? rawUpiQrImage
          : String(rawUpiQrImage?.url || "")

      setForm({
        accountHolderName: String((pending.accountHolderName ?? doc.accountHolderName) || ""),
        accountNumber,
        confirmAccountNumber: accountNumber,
        ifscCode: String((pending.ifscCode ?? doc.ifscCode) || "").toUpperCase(),
        upiId: String((pending.upiId ?? doc.upiId) || ""),
        upiQrImage,
      })
