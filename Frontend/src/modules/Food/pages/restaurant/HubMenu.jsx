...
export default function HubMenu() {
  ...
  const handleSaveAddon = async () => {
    if (addonImages.length === 0) {
      toast.error("Please add at least one image for the add-on")
      return
    }
    if (!addonName.trim()) {
  ...
  const handleContinueAddCategory = async () => {
    ...
    toast.message('Finish category setup on Menu Categories and choose veg or non-veg diet scope before admin approval.')
  ...
  return (
    <div className="h-screen bg-gray-50 flex flex-col overflow-hidden">
      ...
      <div className={`fixed right-4 bottom-24 z-30 flex flex-col gap-1 ${isKeyboardOpen ? 'hidden' : ''}`}>
        {/* ADD Button */}
