...
export default function Inventory() {
  ...
  const handleSaveAddon = async () => {
    if (!addonImageFile) {
      toast.error("Please add an image for the add-on")
      return
    }
    if (!addonName.trim()) {
  ...
  return (
    <div className="min-h-screen bg-[#f3f5f8] flex flex-col lg:h-screen lg:overflow-hidden">
      ...
      <div className="bg-[#f3f5f8] px-4 lg:px-0 lg:bg-white lg:border-b lg:border-slate-200 pt-4 pb-4 lg:py-0">
        ...
        <div ref={tabBarRef} className="flex lg:inline-flex lg:bg-slate-100 lg:rounded-xl lg:p-1 grid lg:grid-cols-none grid-cols-2 gap-3 lg:gap-1">
          ...
          >
            ...
            <span className="relative z-10 flex items-center justify-center gap-2">
              <span>Add-ons</span>
              <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${activeTab === "add-ons" ? "bg-white lg:bg-emerald-50 lg:text-emerald-700 text-slate-950" : "bg-slate-100 text-slate-600"
                }`}>
                {addons.length}
              </span>
            </span>
        ...
        <div className="hidden lg:flex flex-1 items-center gap-3 min-w-0">
          <div className="flex-1 relative min-w-0">
            ...
            <input
              ...
              placeholder={activeTab === "add-ons" ? "Search add-ons" : "Search items"}
      ...
      >
        ...
        {/* Mobile Categories Accordions (Hidden on Desktop) */}
        <div className="lg:hidden space-y-4 mb-6 px-4">
          {activeTab === "add-ons" && (
            <>
              {/* Inline form removed. Using popup modal instead. */}
              {loadingAddons ? (
                ...
                <div className="space-y-4">
                  {filteredAddons.map((addon) => (
                    <div
                      key={addon.id}
                      className="rounded-xl border border-white/80 bg-white p-2.5 sm:p-4 shadow-[0_20px_48px_-34px_rgba(15,23,42,0.45)]"
                    >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="mb-2 flex items-center gap-2 flex-wrap">
                            <div className="flex items-center gap-2">
                              ...
                              <h3 className="text-sm sm:text-base font-medium sm:font-semibold text-slate-950">{addon.name}</h3>
                            </div>
                            <span className={`rounded-full px-2 py-0.5 text-[10px] sm:text-[11px] font-medium sm:font-semibold ${addon.isAvailable !== false
                            ...
                            {addon.approvalStatus === 'approved' && (
                              <span className="rounded-full bg-green-100 px-2 py-0.5 text-[10px] sm:text-[11px] font-medium sm:font-semibold text-green-800">Approved</span>
                            )}
                            {addon.approvalStatus === 'pending' && (
                              <span className="rounded-full bg-yellow-100 px-2 py-0.5 text-[10px] sm:text-[11px] font-medium sm:font-semibold text-yellow-800">Pending</span>
                            )}
                            {addon.approvalStatus === 'rejected' && (
                              <span className="rounded-full bg-red-100 px-2 py-0.5 text-[10px] sm:text-[11px] font-medium sm:font-semibold text-red-800">Rejected</span>
                            )}
                          </div>
                          {addon.description && (
                            <p className="mb-2 text-sm leading-6 text-slate-600">{addon.description}</p>
                          )}
                          <p className="text-sm sm:text-base font-semibold sm:font-bold text-slate-950">Rs. {addon.price}</p>
                        ...
                        <div className="flex items-start gap-3">
                          {addon.images && addon.images.length > 0 && addon.images[0] && (
                            <img
                              src={addon.images[0]}
                              alt={addon.name}
                              className="h-16 w-16 sm:h-20 sm:w-20 rounded-xl sm:rounded-2xl object-cover ring-1 ring-slate-200"
                              onError={(e) => {
      ...
      <AnimatePresence>
        {isAddAddonOpen && (
          <>
            ...
            />
            <div className="fixed inset-0 z-[71] flex items-center justify-center p-4 lg:p-6 pointer-events-none">
              <motion.div
                initial={{ opacity: 0, scale: 0.96, y: 12 }}
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.96, y: 12 }}
      ...
      <div className={`fixed right-4 bottom-24 z-30 flex flex-col items-end gap-2 lg:hidden ${isKeyboardOpen ? 'hidden' : ''}`}>
        {activeTab === "add-ons" ? (
          <motion.button
            whileTap={{ scale: 0.96 }}
            onClick={() => {
      ...
      <div className={`lg:hidden ${isKeyboardOpen ? 'hidden' : ''}`}>
        <BottomNavOrders />
      </div>
    </div>
  )
