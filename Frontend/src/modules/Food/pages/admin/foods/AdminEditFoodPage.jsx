...
export default function AdminEditFoodPage() {
  ...
  useEffect(() => {
    ...
    const loadFood = async () => {
      try {
        ...
        const existingFood = response?.data?.data?.food || response?.data?.food || null
        ...
        if (existingFood) {
          setFormData({
            ...
            variations: existingFood.variants || existingFood.variations || [],
            tags: existingFood.tags || [],
            nutrition: existingFood.nutrition || [],
            allergies: existingFood.allergies || [],
            restaurantId: String(existingFood.restaurantId || ""),
          })
        }
      } catch {
        if (isMounted) {
  ...
  useEffect(() => {
    ...
    const loadCategories = async () => {
      if (!formData.restaurantId) {
        if (isMounted) setAvailableCategories([])
        return
      }
      try {
        const response = await adminAPI.getRestaurantCategoriesForMenu(formData.restaurantId)
  ...
  }, [isNewFood, formData.restaurantId])
  ...
  useEffect(() => {
    ...
    const selectedRestaurant = restaurants.find(r => String(r.id || r._id) === String(formData.restaurantId))
    const pureVeg = selectedRestaurant?.pureVegRestaurant === true
    setIsPureVegRestaurant(pureVeg)
    if (pureVeg && formData.foodType !== "Veg") {
      setFormData((prev) => ({ ...prev, foodType: "Veg" }))
    }
  }, [formData.restaurantId, restaurants, formData.foodType])
  ...
  const handleSubmit = async (e) => {
    ...
    if (isNewFood) {
      try {
        const res = await adminAPI.createFood({
          ...foodDataToSave,
    ...
    try {
      const res = await adminAPI.updateFood(String(id), {
        ...foodDataToSave,
        categoryId: matchedCategory.id,
        categoryName: matchedCategory.name,
      })
  ...
  return (
    <div className="bg-[#f6e9dc] min-h-full pb-20">
      {/* Header */}
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="px-4 py-3 flex items-center gap-4">
          <button 
            onClick={() => navigate(`/admin/food/foods`)}
      ...
      {/* Content */}
      <form id="edit-food-form" onSubmit={handleSubmit} className="pb-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    disabled={!isNewFood}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff8100] focus:border-transparent outline-none disabled:bg-gray-100"
                    required
                  >
      ...
      {/* Fixed Bottom Button */}
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-50 md:relative md:border-0 md:shadow-none mt-6">
