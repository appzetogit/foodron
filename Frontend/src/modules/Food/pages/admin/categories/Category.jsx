...
export default function Category() {
  ...
  const filteredCategories = useMemo(() => {
    // Mirrors the user-side ordering: display order starts at 1, and legacy
    // categories still on 0 have no explicit position so they sit at the end.
    const displayRank = (category) => {
      const value = Number(category?.sortOrder)
      return Number.isFinite(value) && value > 0 ? value : Number.MAX_SAFE_INTEGER
    }
    const list = [...categories].sort((a, b) => displayRank(a) - displayRank(b))
  ...
  const handleEdit = (category) => {
    ...
    setFormData({
      ...
      sortOrder: Math.max(1, Number.isFinite(Number(category?.sortOrder)) ? Number(category.sortOrder) : 1),
    })
    setSelectedImageFile(null)
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const payload = {
        ...
        sortOrder: Math.max(1, Number.isFinite(Number(formData.sortOrder)) ? Number(formData.sortOrder) : 1),
      }
                      ...
                      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-5">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="1"
                          />
                          <p className="mt-2 text-xs text-slate-500">Starts from 1. Lower numbers appear first on the user side.</p>
                        </div>

                        <div>
                          <label className="mb-2 block text-sm font-medium text-slate-700">
                            Category Image <span className="text-rose-600">*</span>
                          </label>
                          <div className="space-y-3">
                            ...
                            {!imagePreview && !formData.image && (
                              <p className="text-xs text-rose-600">An image is required before saving the category.</p>
                            )}
                          </div>
                        </div>
