            ...
            <div className="bg-white rounded-lg shadow-sm border border-[#e3e6ef] p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  {Number(paymentSummary?.menuDiscount) > 0 && (
                    <>
                      <div className="flex justify-between items-center py-2 border-b border-[#e3e6ef]">
                        <span className="text-sm text-[#8a94aa]">
                          Menu Discount{Number(paymentSummary?.menuDiscountPercentage) > 0 ? ` (${paymentSummary.menuDiscountPercentage}%)` : ""}
                        </span>
                        <span className="text-sm font-semibold text-[#334257]">{formatCurrency(paymentSummary.menuDiscount)}</span>
                      </div>
                      <div className="flex justify-between items-center py-2 border-b border-[#e3e6ef]">
                        <span className="text-sm text-[#8a94aa]">↳ Borne by Admin</span>
                        <span className="text-sm font-semibold text-[#334257]">{formatCurrency(paymentSummary.menuDiscountAdminShare || 0)}</span>
                      </div>
                      <div className="flex justify-between items-center py-2 border-b border-[#e3e6ef]">
                        <span className="text-sm text-[#8a94aa]">↳ Borne by Restaurant</span>
                        <span className="text-sm font-semibold text-[#334257]">{formatCurrency(paymentSummary.menuDiscountRestaurantShare || 0)}</span>
                      </div>
                    </>
                  )}
