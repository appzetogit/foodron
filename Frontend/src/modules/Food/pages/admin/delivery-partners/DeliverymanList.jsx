...
import { Search, Download, ChevronDown, Eye, User, Star, ArrowUpDown, Settings, FileText, FileSpreadsheet, Loader2, Check, Columns, ExternalLink, Calendar, MapPin, CreditCard, Mail, Phone, Bike, FileCheck, Pencil, Save, X, Plus, ChevronLeft, ChevronRight } from "lucide-react"
...
export default function DeliverymanList() {
  ...
  const canCreate = useMemo(() => {
    return canPerformAdminPermissionAction(currentUser, resolvedPermissions, "food::deliveryman_management::deliveryman::list", "create")
  }, [currentUser, resolvedPermissions])
  ...
  // Fetch delivery partners + wallets for the same page only (limit 30).
  const fetchDeliverymen = useCallback(async (opts = {}) => {
    const nextPage = Number(opts.page) > 0 ? Number(opts.page) : page
    const nextSearch =
      typeof opts.search === "string" ? opts.search : searchQuery
    ...
    try {
      ...
      const [partnersResponse, walletsResponse] = await Promise.allSettled([
        ...
        adminAPI.getDeliveryBoyWallets({
      ...
      if (partnersResponse.status === "fulfilled" && partnersResponse.value?.data?.success) {
        ...
        setPagination({
          page: Number(pager.page) || nextPage,
          limit: Number(pager.limit) || PAGE_SIZE,
          total: Number(pager.total) || partners.length,
  ...
  // One fetch path only — avoids mount + search double-call.
  useEffect(() => {
    const delay = searchQuery.trim() ? 400 : 0
    const timer = setTimeout(() => {
      fetchDeliverymen({ page, search: searchQuery })
    }, delay)
    return () => clearTimeout(timer)
    // fetchDeliverymen closes over page/search; depend on those primitives only.
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="px-3 py-1 rounded-full text-sm font-semibold bg-slate-100 text-slate-700">
                {pagination.total || filteredDeliverymen.length}
              </span>
            </div>
          </div>
          ...
          {error && (
            <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
              ...
              <button
                onClick={() => fetchDeliverymen({ page, search: searchQuery })}
            ...
            <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-1 px-3 py-2 text-sm rounded-lg border border-slate-300 bg-white hover:bg-slate-50 disabled:opacity-50"
                >
