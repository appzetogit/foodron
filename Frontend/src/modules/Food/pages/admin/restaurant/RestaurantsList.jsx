...
export default function RestaurantsList() {
  ...
  const handleToggleVisibility = async (restaurant) => {
    ...
    try {
      ...
      await adminAPI.toggleRestaurantListing(restaurantId, newStatus);
      
      setRestaurants(prev => 
        prev.map(r => 
          (r.id === restaurant.id || r._id === restaurant._id)
            ? { ...r, isListed: newStatus }
            : r
        )
      );
      toast.success(`Restaurant is now ${newStatus ? 'visible' : 'invisible'}`);
    } catch (err) {
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className={`w-fit mt-1 px-2 py-0.5 rounded text-[10px] font-semibold flex items-center gap-1 ${(restaurant.originalData?.pureVegRestaurant === true || String(restaurant.originalData?.pureVegRestaurant).toLowerCase() === 'true') ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                                <span className={`inline-block w-1.5 h-1.5 rounded-full ${(restaurant.originalData?.pureVegRestaurant === true || String(restaurant.originalData?.pureVegRestaurant).toLowerCase() === 'true') ? 'bg-green-600' : 'bg-amber-600'}`}></span>
                                {(restaurant.originalData?.pureVegRestaurant === true || String(restaurant.originalData?.pureVegRestaurant).toLowerCase() === 'true') ? 'Pure Veg' : 'Mixed'}
                              </span>
                            </div>
                          </div>
                        </td>
