...
const DeliveryTrackingMap = ({
  ...
  const tripStatus = order?.status || order?.orderStatus || order?.deliveryState?.currentPhase || 'pending';
  const isOrderPickedUp = ['picked_up', 'out_for_delivery', 'on_way', 'en_route_to_delivery', 'at_drop', 'reached_drop', 'delivered'].includes(String(tripStatus).toLowerCase());
  ...
  useEffect(() => {
    ...
    if (isOrderPickedUp) {
      ...
      else if (customerCoords) bounds.extend(customerCoords);
    }
    ...
    <div className="relative w-full h-full overflow-hidden rounded-2xl shadow-inner border border-gray-100">
      ...
      >
        ...
        >
          <div className="relative -translate-x-1/2 -translate-y-full mb-1 group">
             ...
             {!isOrderPickedUp && (
               <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                 <motion.div 
                   ...
                   className="w-16 h-16 rounded-full border-4 border-red-500/50"
                 />
               </div>
             )}
             <div className="relative w-11 h-11 rounded-full p-1 bg-white shadow-xl border-2 border-red-500 overflow-hidden group-hover:scale-110 transition-transform">
                <img 
             ...
             {/* Pin Tip */}
             <div className="absolute top-[100%] left-1/2 -translate-x-1/2 w-3 h-3 bg-red-500 clip-triangle rotate-180 -mt-1 shadow-sm" style={{ clipPath: 'polygon(50% 100%, 0 0, 100% 0)' }} />
          </div>
        </OverlayView>
      ...
      <AnimatePresence>
        {riderLocation && currentEta && (
          ...
          >
            <div className="bg-red-500/95 backdrop-blur-xl rounded-2xl p-3 shadow-[0_10px_30px_rgba(249,115,22,0.4)] border border-red-400/50 flex flex-col min-w-[90px] group overflow-hidden relative">
