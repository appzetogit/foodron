...
export default function RestaurantNavbar({
  ...
  // Check if open based on timings
  useEffect(() => {
    const checkIfOpen = () => {
  ...
  const displayStatus = (status === "Online" && isWithinTimings !== false) ? "Online" : "Offline"
    ...
    <div className="w-full bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
      ...
      <div className="flex items-center">
        ...
        {showOfflineOnlineTag && (
          ...
          >
            <span className={`w-1.5 h-1.5 rounded-full ${displayStatus === "Online" ? "bg-[#FF0000]" : "bg-gray-500"
              }`}></span>
            <span className={`text-sm font-medium ${displayStatus === "Online" ? "text-[#FF0000]" : "text-gray-700"
              }`}>
              {displayStatus}
            </span>
            <ChevronRight className={`w-4 h-4 ${displayStatus === "Online" ? "text-[#FF0000]" : "text-gray-700"
              }`} />
          </button>
        )}
