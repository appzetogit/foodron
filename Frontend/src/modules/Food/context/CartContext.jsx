...
const getItemOrderType = () => "food"
const getItemSourceId = (item) =>
  String(item?.sourceId || item?.restaurantId || item?.sourceRestaurantId || "")
  ...
  return rawCart
    ...
    .map((item, index) => {
      ...
      return {
        ...
        type: orderType,
        sourceId,
        sourceName: item.sourceName || normalizedRestaurantName,
        quantity:
...
export function CartProvider({ children }) {
  ...
  const addToCart = useCallback((item, sourcePosition = null) => {
    ...
    setCart((prev) => {
      ...
      const resolvedItemId = resolveCartEntryId(safePrev, item.id || item.itemId || item._id, item.variantId)
      ...
      if (existing) {
        ...
        return safePrev.map((i) => i.id === resolvedItemId ? { ...i, quantity: i.quantity + 1 } : i)
      }
