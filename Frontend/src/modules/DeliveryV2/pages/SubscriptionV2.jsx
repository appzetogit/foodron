...
export default function SubscriptionV2() {
  ...
  const getPlanIcon = (unit) => {
    switch (unit) {
      ...
      case 'MONTH': return <Trophy className="w-5 h-5 text-red-500" />;
      default: return <Crown className="w-5 h-5 text-purple-500" />;
    }
  }

  return (
    <div className="app-shell-page min-h-screen bg-[#F8FAFC]">
      {/* Premium Header */}
      <div className="app-shell-page__header safe-top bg-white px-4 py-5 flex items-center gap-4 shadow-sm border-b border-slate-100">
      ...
      <div className="app-shell-page__body pb-24 p-4 space-y-8 max-w-md mx-auto">
        ...
        <div className="space-y-4">
          ...
          <AnimatePresence>
            {activeSub ? (
              ...
              >
                <div className="absolute inset-0 bg-gradient-to-r from-red-600 to-rose-600 rounded-[32px] blur-xl opacity-20 group-hover:opacity-30 transition-opacity" />
        ...
        <div className="relative group">
          <div className="absolute inset-0 bg-red-500 rounded-[32px] blur-xl opacity-5 group-hover:opacity-10 transition-opacity" />
              ...
              <div className="divide-y divide-slate-50">
                {ledger.map((entry) => (
                  <div key={entry._id} className="p-4 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                    <div className="flex items-center gap-4">
                      <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${entry.type === 'TOPUP' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
